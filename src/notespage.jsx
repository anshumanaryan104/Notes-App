...
function Sidebar({setSelectedNote}){
  ...
  return (
    <div className='w-1/4 bg-blue-500 text-white p-4 rounded-md '>
      <h3 className="font-bold mb-4">Your Notes</h3>
      {notes.length > 0 ? (
        notes.map((note) => (
          <div key={note.note_id} className="mb-2 p-2 bg-blue-600 rounded">
...
function NotesPage() {
  ...
  return (
    ...
    <div className='w-full min-h-screen bg-black'>
        <h1 className='text-center text-white text-3xl font-bold py-4'>Notes APP</h1>
        <div className='px-5 pb-5 flex justify-center items-center gap-4'>
          {selectedNote && (
            <>
            <button className='bg-blue-300 text-white px-5 py-3 rounded-md' onClick={()=>setSelectedNote(null)}>Close</button>
            </>
          )}
            <button className='bg-blue-300 text-white px-5 py-3 rounded-md' onClick={addNotes}>Save Notes</button>
            <input type="text" placeholder="add title" className='rounded-md py-2 text-center w-64' value={note_title} onChange={(e)=>setNote_title(e.target.value)}/>
      ...
      <div className='flex w-full gap-4 px-4 min-h-screen'>
        ...
        {selectedNote ? (
          <div className='w-3/4 bg-gray-800 p-4 rounded-md text-white'>
            <h2 className='text-2xl font-bold mb-4'>{selectedNote.note_title}</h2>
            <p className='whitespace-pre-wrap'>{selectedNote.note_text}</p>
          </div>
        
        ) : (
          <input type="text" id='notes' placeholder="Add your notes" className='w-3/4 top-0 py-2 text-center rounded-md' value={note_text} onChange={(e)=>setNote_text(e.target.value)}/>
        )}
      </div>
    </div>
  )
